...
## Important:
 ...
 - This algorithm can process an input up to 511 MB in size, which should be sufficient for most use cases. The limit is caused by the size limits of an unsigned 32-bit integer `input_size[0]`. To be precise the max size is 4294966144 bits (2^32 - 1 *(padding bit)* - 128 *(length bits)* - 1023 *(needed for ceil division)*).
